...
def run(args: list[str], timeout: int = 30, *, binary: bool = False):
    ...
    try: completed = subprocess.run(args, capture_output=True, timeout=timeout, check=False)
    except FileNotFoundError as exc: raise RuntimeError(f"Command not found: {exc.filename}") from exc
