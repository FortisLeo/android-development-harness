...
from .config import allowed_project_path

def build(project_dir: str, tasks: list[str] | None=None, timeout: int=900) -> dict:
    project=allowed_project_path(project_dir,must_exist=True); wrapper=project/("gradlew.bat" if os.name=="nt" else "gradlew")
...
def debug_apk(project_dir: str) -> dict:
    project=allowed_project_path(project_dir,must_exist=True); apks=sorted((project/"app"/"build"/"outputs"/"apk").glob("**/*-debug.apk"))
